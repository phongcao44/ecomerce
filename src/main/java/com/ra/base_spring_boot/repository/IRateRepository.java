...
public interface IRateRepository extends JpaRepository<Review, Long> {
    ...
    @Query("SELECT new com.ra.base_spring_boot.dto.resp.ListProductReviewResponse(" +
            "p.id, p.name, AVG(r.rating), COUNT(r.id)) " +
            "FROM Review r " +
            "JOIN r.product p " +
            "GROUP BY p.id, p.name")
    List<ListProductReviewResponse> findReviewSummariesGroupedByProduct();
