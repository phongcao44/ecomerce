...
public class ShippingFeeServiceImpl implements ShippingFeeService {
    ...
    @Override
    public ShippingFee calculateShippingFeeByAddressId(Long addressId) {
        Address address = addressRepository.findById(addressId)
                .orElseThrow(() -> new RuntimeException("Không tìm thấy địa chỉ với ID: " + addressId));
        return calculateAndSaveShippingFee(address);
    }

    public ShippingFee calculateAndSaveShippingFee(Address address) {
        int serviceId = getServiceId(address.getDistrictId());
